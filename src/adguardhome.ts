...
export default class AdGuardHomeServer {
    ...
    : Promise<AdGuardStatus>
  {
    ...
    let abortPromise: boolean = false;
    await Promise.all([
      getStatus ? this.aghApi("status")
        .json()
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        .then((body: any) => { if (!abortPromise) { this.latest.enabled = currentStatus.enabled = body.protection_enabled; } })
        : Promise.resolve(),

      getDefaultServices ? this.aghApi("blocked_services/get")
        .json()
        // // eslint-disable-next-line @typescript-eslint/no-explicit-any
        .then((body: any) => { if (!abortPromise) { this.latest.blocked_services = currentStatus.blocked_services = body.ids; } })
        : Promise.resolve(),

      getDisallowedClients ? this.aghApi("access/list")
        .json()
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        .then((body: any) => { if (!abortPromise) { this.latest.blocked_clients = currentStatus.blocked_clients = body.disallowed_clients; } })
        : Promise.resolve(),

      getClientServices ? this.aghApi("clients")
        .json()
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        .then((body: any) => { if (!abortPromise) { this.latest.clients = currentStatus.clients = body.clients; } })
        : Promise.resolve(),
    ])
    .catch((error) => {
      abortPromise = true;
  ...
  private async doPostWrapper(post: Promise<any>): Promise<boolean>
  {
    ...
    var success: boolean = await post.then((result) => {
      ...
      if (Array.isArray(result))
      {
        ...
        if (failed.length > 0)
        {
          let failedResponses = failed.map(p => p.reason); // Get actual results out of PromiseSettledResult<T>
      ...
      if (result.request) this.log.debug(`AGH: Headers == ${result.request.headers}`);
      this.log.error(result.response ? result.response.body : result);
      return false;
    })
    .catch((error) => {
      this.log.info(`AGH: Request failed with an error: ${error.code}`);
      if (error.request) this.log.debug(`AGH: Headers == ${error.request.headers}`);
  ...
  private merge(list1: string[], list2: string[], sort: boolean = false): string[]
  {
    const newList: string[] = [];
    list1.forEach((s) => newList.push(s));
    list2.forEach((s) => { if (!newList.includes(s)) newList.push(s); });
  ...
  {
    const newList: string[] = [];
    oldList.forEach((s) => { if (!subList.includes(s)) newList.push(s); });
    return newList;
  }
}
