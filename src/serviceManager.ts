...
export default class ServiceManager {
  ...
  {
    let timerString: string = (timeout == 0) ? 'No Timer' : `${timeout} Minute${timeout > 1 ? 's' : ''}`;
    let configuredName: string = `${this.group.name} - ${timerString}`;
    let displayName: string = concise ? timerString : `${this.group.name}${(timeout == 0) ? '' : ': ' + timerString}`;
    let subtype: string = (timeout == 0) ? 'main_lock' : `timed_lock_${timeout}`;

    this.log.info(`SM: Creating AdGuard Home Accessory:Service ${accessory.displayName}:${configuredName}[${subtype}](${this.serviceType}).`);
    ...
    switch (this.serviceType) {
      ...
      case ServiceType.TV:
        ...
        var inputName = `🟢${AGHState.BLOCKING}`;
        ...
        inputBlocking.setCharacteristic(this.Characteristic.Identifier, 1)
          ...
          .setCharacteristic(this.Characteristic.IsConfigured, this.Characteristic.IsConfigured.NOT_CONFIGURED) // NOT_CONFIGURED == not selectable in HK 👍
    ...
    if (this.serviceType === ServiceType.TV)
    {
      ...
      service.getCharacteristic(this.Characteristic.Active)
        ...
        .onSet(async (/* value: CharacteristicValue */) => {
          ...
          setTimeout(() => { service.updateCharacteristic(this.Characteristic.Active, this.Characteristic.Active.ACTIVE); });
        });
      ...
      service.getCharacteristic(this.Characteristic.ActiveIdentifier)
        ...
        .onSet(async (/* value: CharacteristicValue */) => {
          setTimeout(() => service.updateCharacteristic(this.Characteristic.ActiveIdentifier, this.toCharacteristicValue(this.group.currentState)));
        });
    }
  ...
  public toCharacteristicValue(state: string, throwIfUnavailable: boolean = false): CharacteristicValue
  {
